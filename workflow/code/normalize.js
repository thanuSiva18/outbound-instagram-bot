...
// always "Zayn" (warm, friendly travel consultant).
//
...
//
// The system prompt body is maintained in prompts/system_prompt.md and inlined
// below as a template literal. ${notesBlock}, the PRIOR_CHAT / RETURNING_USER /
// LEAD_ALREADY_COMPLETE flags, ${knownJson}, ${WA} and ${WA_LINK} interpolate
// at runtime.
...
const priorChat = anyKnown || hasNotes;
...
// Outbound Travelers — Instagram DM Agent (System Prompt v3). Source of truth:
// prompts/system_prompt.md. Keep the two in sync.
...
PRIOR_CHAT: ${priorChat ? 'yes' : 'no'}  — yes means you have ALREADY talked with this person (records or notes exist). Treat ANY earlier message visible above as prior chat too, even if this flag says no.
RETURNING_USER: ${anyKnown ? 'yes' : 'no'}  — yes means we already have SAVED FIELD VALUES for this person.
...
{"reply":"Love it Thanu — China for 10 of you sounds amazing 😊 last thing, what's your WhatsApp number so our expert can share all the details?","intent":"travel_lead","fields":{"name":"Thanu","whatsapp_number":"","destination":"China","pax":"10","budget":"medium / flexible"},"notes":"Thanu, China, 10 pax, medium/flexible budget. Ignored date/duration noise per the date rule. Next: WhatsApp number.","status":"in_progress"}
